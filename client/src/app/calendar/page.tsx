...
export default function CalendarPage() {
    ...
    useEffect(() => {
        import('@/lib/api').then(({ apiFetch }) => {
            apiFetch('/events').then(res => res.json()).then(data => {
    ...
    // Filter for upcoming events (date >= today's date)
    // We construct 'today' at midnight to compare properly string YYYY-MM-DD
    const todayStr = format(new Date(), 'yyyy-MM-dd');
    const upcomingEvents = events
        .filter(e => e.date >= todayStr)
        .sort((a, b) => a.date.localeCompare(b.date));

    // const dayEvents = events.filter(e => e.date === selectedDateStr); // Old logic
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-rose-50 via-white to-rose-100 pb-24 p-4">
            ...
            <div className="max-w-2xl mx-auto space-y-6">
                <GlassCard className="p-6 !rounded-3xl bg-white/80 backdrop-blur-xl shadow-xl shadow-rose-200/50 border border-white/60">
                    <style>{`
                        .react-calendar { 
                ...
                <div className="space-y-4">
                    <div className="flex items-center justify-between">
                        ...
                        >
                            <Plus className="w-4 h-4 mr-1" /> Add Plan ({format(date, 'MMM d')})
                        </Button>
                    </div>
                        ...
                        {isAdding && (
                            ...
                            >
                                <input
                                    ...
                                    placeholder={`Plan for ${format(date, 'MMM d')}...`}
                    ...
                    <div className="space-y-3">
                        {upcomingEvents.length === 0 && !isAdding && (
                            <p className="text-center text-rose-900/30 py-4 italic">No upcoming plans.</p>
                        )}
                        {upcomingEvents.map(event => (
                            <GlassCard key={event._id} className="items-center flex justify-between p-4">
                                <div className="flex flex-col">
                                    ...
                                    <span className="text-xs text-rose-500 font-bold uppercase">{format(new Date(event.date), 'MMMM d')}</span>
                                </div>
